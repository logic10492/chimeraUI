...
function RuntimeSection({ config, setConfig, lang }: SectionProps) {
  ...
  const fields: FieldDef[] = [
    ...
    { key: 'compaction.remote', label: 'compaction.remote', desc: tx('Remote compaction mode: auto, on, or off.', '远程压缩模式：auto、on 或 off。', lang), control: <Select value={compaction.remote} options={enumChoices(['auto', 'on', 'off'])} onChange={v => setConfig(setNested(config, ['compaction', 'remote'], v))} /> },
    { key: 'compaction.remote_protocol', label: 'compaction.remote_protocol', desc: tx('Remote compaction protocol: auto, v2, or legacy.', '远程压缩协议：auto、v2 或 legacy。', lang), control: <Select value={compaction.remote_protocol} options={enumChoices(['auto', 'v2', 'legacy'])} onChange={v => setConfig(setNested(config, ['compaction', 'remote_protocol'], v))} /> },
