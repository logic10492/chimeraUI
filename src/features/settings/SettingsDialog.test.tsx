...
vi.mock('./components/CompactionSettings', () => ({
  CompactionSettings: () => (
    <div>
      <div data-setting-label="Project Policy"><button type="button">Compaction policy control</button></div>
      <div data-setting-label="Provider & Model Eligibility"><button type="button">Eligibility control</button></div>
      <div data-setting-label="Technical Details"><button type="button">Resolved status control</button></div>
    </div>
  ),
}))
...
vi.mock('./components/ProviderSettings', () => ({
  ProviderSettings: () => (
    <div>
      <div data-setting-label="Connected providers"><button type="button">Connected providers control</button></div>
      <div data-setting-label="Available providers"><button type="button">Available providers control</button></div>
    </div>
  ),
}))
...
describe('SettingsDialog search', () => {
  ...
  it('jumps to a highlighted result in the local compaction tab', async () => {
    render(<SettingsDialog isOpen onClose={vi.fn()} />)
    await act(async () => vi.advanceTimersByTime(1))

    const input = screen.getByRole('combobox', { name: 'Search settings' })
    fireEvent.focus(input)
    fireEvent.change(input, { target: { value: 'Project Policy' } })
    fireEvent.click(screen.getByRole('option', { name: /Project Policy/ }))
    await act(async () => vi.advanceTimersByTime(1))

    expect(screen.getByRole('tab', { name: 'Compaction' })).toHaveAttribute('aria-selected', 'true')
    expect(screen.getByText('Compaction policy control').parentElement).toHaveClass('settings-search-highlight')
    expect(screen.getByRole('button', { name: 'Compaction policy control' })).toHaveFocus()
  })

  it('jumps to a highlighted result in the local providers tab', async () => {
    render(<SettingsDialog isOpen onClose={vi.fn()} />)
    await act(async () => vi.advanceTimersByTime(1))

    const input = screen.getByRole('combobox', { name: 'Search settings' })
    fireEvent.focus(input)
    fireEvent.change(input, { target: { value: 'Available providers' } })
    fireEvent.click(screen.getByRole('option', { name: /Available providers/ }))
    await act(async () => vi.advanceTimersByTime(1))

    expect(screen.getByRole('tab', { name: 'Providers' })).toHaveAttribute('aria-selected', 'true')
    expect(screen.getByText('Available providers control').parentElement).toHaveClass('settings-search-highlight')
    expect(screen.getByRole('button', { name: 'Available providers control' })).toHaveFocus()
  })
