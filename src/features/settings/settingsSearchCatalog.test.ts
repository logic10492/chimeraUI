...
describe('settings search catalog', () => {
  ...
  it('covers the local compaction tab', () => {
    const compactionTabs = new Set(SETTINGS_SEARCH_DEFINITIONS.filter(item => item.tab === 'compaction').map(item => item.labelKey))
    expect(['compaction.scopeTitle', 'compaction.policy', 'compaction.eligibility', 'compaction.resolvedStatus'].every(key => compactionTabs.has(key))).toBe(true)
  })

  it('covers the local providers tab', () => {
    const providersTabs = new Set(SETTINGS_SEARCH_DEFINITIONS.filter(item => item.tab === 'providers').map(item => item.labelKey))
    expect(['providers.connectedProviders', 'providers.availableProviders'].every(key => providersTabs.has(key))).toBe(true)
  })
})